...
public interface LearnedWordDao {
    ...
    @Query("SELECT * FROM learned_words WHERE LOWER(userEmail) = LOWER(:email) ORDER BY lastSeenAt DESC")
    ...
    @Query("UPDATE learned_words SET timesCorrect = timesCorrect + 1, timesSeen = timesSeen + 1, lastSeenAt = :now WHERE id = :id")
    void markCorrect(int id, long now);

    @Query("UPDATE learned_words SET timesWrong = timesWrong + 1, timesSeen = timesSeen + 1, lastSeenAt = :now WHERE id = :id")
